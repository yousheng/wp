...
add_filter( 'body_class', 'hootdu_body_class_filter', 0, 2 );

# Filters the WordPress 'post_class' early.
add_filter( 'post_class', 'hootdu_post_class_filter', 0, 3 );

# Filters the WordPress 'comment_class' early.
add_filter( 'comment_class', 'hootdu_comment_class_filter', 0, 3 );
...
function hootdu_get_context() {
	...
	return array_map( 'esc_attr', apply_filters( 'hootdu_context', array_unique( $context ) ) );
}
 ...
 * @return array
 */
function hootdu_body_class_filter( $classes, $class ) {
	...
	// Locale and language.
	$locale = get_locale();
	$lang   = hootdu_get_language( $locale );
	...
	if ( apply_filters( 'hootdu_is_plural', is_home() || is_archive() || is_search() ) )
		$classes[] = 'plural';

	// Merge base contextual classes with $classes.
	$classes = array_merge( $classes, hootdu_get_context() );
 ...
 * @return array
 */
function hootdu_post_class_filter( $classes, $class, $post_id ) {
 ...
 * @return array
 */
function hootdu_comment_class_filter( $classes, $class, $comment_id ) {
