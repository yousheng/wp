...
class Hootdu_Customize_Group_Control extends WP_Customize_Control {
	...
	protected function render() {
		...
		$class = 'customize-control customize-control-' . $this->type . ' hootdu-customize-control-' . $this->type . $this->group;
		if ( !empty( $this->identifier ) )
			$class .= ' hootdu-control-id-' . $this->identifier;
		if ( $this->group == 'start' )
			$class .= ' ' . hootdu_sanitize_html_classes( $this->startwrap );
			...
			case 'group' :
				...
				switch ( $this->group ) {
					...
					case 'start' :
						...
						?>
						<div class="button hootdu-flypanel-button" data-flypaneltype="group"><?php echo wp_kses( $button, $buttontags ); ?></div>
					<?php
					break;
...
// Only load in customizer (not in frontend)
if ( class_exists( 'WP_Customize_Control' ) ) :
function hootdu_customize_group_control_interface ( $wp_customize, $id, $setting ) {
		...
		switch( $setting['type'] ) {
			...
			case 'groupstart':
				...
				$wp_customize->add_control(
					new Hootdu_Customize_Group_Control( $wp_customize, $id, $setting )
				);
				break;
...
add_action( 'hootdu_customize_control_interface', 'hootdu_customize_group_control_interface', 10, 3 );
endif;
 ...
 * @return void
 */
function hootdu_customize_prepare_group_settings( $value, $key, $setting, $count ) {
...
add_filter( 'hootdu_customize_prepare_settings', 'hootdu_customize_prepare_group_settings', 10, 4 );
...
// Only load in customizer (not in frontend)
if ( class_exists( 'WP_Customize_Control' ) ) :
function hootdu_customize_footer_groupcontent() {

	?>
	<div id="hootdu-flygroup" class="hootdu-flypanel">
		...
		<div id="hootdu-flygroup-content" class="hootdu-flypanel-content">
		</div>
...
add_action( 'customize_controls_print_footer_scripts', 'hootdu_customize_footer_groupcontent' );
endif;