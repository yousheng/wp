...
class Hootdu_Customize_Sortlist_Control extends WP_Customize_Control {
	...
	public function render_content() {
		...
		switch ( $this->type ) {
			...
			case 'sortlist' :
				...
				?>
				<ul id="hootdu-control-sortlist-<?php echo esc_attr( $id ) ?>" class="hootdu-control-sortlist <?php if ( $sortable ) echo 'sortable'; ?>" <?php echo $open; ?>>
					<?php foreach ( $valuearray as $choiceid => $choicevalues ) : ?>

						<li id="hootdu-control-sortlist-<?php echo esc_attr( $choiceid ) ?>" class="hootdu-control-sortlistitem <?php echo ( !$hideable || empty( $choicevalues['sortitem_hide'] ) ? '' : 'deactivated' ) ?>" data-choiceid="<?php echo esc_attr( $choiceid ) ?>">

							<?php if ( $display ) : ?>
								<span class="hootdu-sortlistitem-head">
									<?php if ( $sortable ) : ?>
									...
									<input class="hootdu-control-sortlistitem-hide" name="<?php echo esc_attr( $id.'['.$choiceid.']'.'[sortitem_hide]' ); ?>" value="<?php echo esc_attr( $hidden ); ?>" type="hidden"/>
							...
							<?php if ( !empty( $this->options[$choiceid] ) && is_array( $this->options[$choiceid] ) ) : ?>
								<span class="hootdu-sortlistitem-options">
									<?php
									foreach ( $this->options[$choiceid] as $optionid => $option ) :
										if ( isset( $option['type'] ) ) { ?>

											<span class="hootdu-sortlistitem-option hootdu-sortlistitem-option-<?php echo sanitize_html_class( $option['type'] ); ?>"><?php

												if ( !empty( $option['label'] ) )
													echo '<span class="hootdu-sortlistitem-option-title">' . esc_html( $option['label'] ) .'</span>';
												if ( !empty( $option['sublabel'] ) )
													echo '<span class="hootdu-sortlistitem-option-sublabel description">' . wp_kses_post( $option['sublabel'] ) . '</span>';
													...
													case 'radioimage': ?>
														...
														if ( !empty( $option['choices'] ) && is_array( $option['choices'] ) ) {
															foreach ( $option['choices'] as $skey => $simage ) {
																...
																<label class="hootdu-customize-radioimage<?php if ($checked) echo ' radiocheck' ?>">
														...
														break;
													case 'content':
														?><span id="<?php echo sanitize_html_class( $id.'_'.$choiceid.'_'.$optionid ); ?>" class="<?php echo sanitize_html_class( $id.'_'.$optionid ); ?> hootdu-customize-control-sortlist-content"><?php echo wp_kses_post( $option['content'] ); ?></span><?php
														break;
												endswitch; ?>
				...
				<input class="hootdu-customize-control-sortlist" value='<?php echo esc_attr( $this->value() ) ?>' <?php $this->link(); ?> type="hidden"/>
				<?php
...
// Only load in customizer (not in frontend)
if ( class_exists( 'WP_Customize_Control' ) ) :
function hootdu_customize_sortlist_control_interface ( $wp_customize, $id, $setting ) {
	if ( isset( $setting['type'] ) ) :
		if ( $setting['type'] == 'sortlist' || $setting['type'] == 'repeatable' ) {
			$setting['type'] = 'sortlist';
			$wp_customize->add_control(
				new Hootdu_Customize_Sortlist_Control( $wp_customize, $id, $setting )
			);
		}
	endif;
}
add_action( 'hootdu_customize_control_interface', 'hootdu_customize_sortlist_control_interface', 10, 3 );
endif;
 ...
 * @return void
 */
function hootdu_customize_prepare_sortlist_settings( $value, $key, $setting, $count ) {
...
add_filter( 'hootdu_customize_prepare_settings', 'hootdu_customize_prepare_sortlist_settings', 10, 4 );
 ...
 * @return string
 */
function hootdu_customize_sanitize_sortlist_callback( $callback, $type, $setting, $name ) { 
	if ( $type == 'sortlist' || $type == 'repeatable' )
		$callback = 'hootdu_sanitize_customize_sortlist';
	return $callback;
}
add_filter( 'hootdu_customize_sanitize_callback', 'hootdu_customize_sanitize_sortlist_callback', 5, 4 );
 ...
 */
function hootdu_sanitize_customize_sortlist( $value, $setting ) {
	...
	if ( !empty( $valuearray ) && is_array( $valuearray ) ) {
		...
		$settings = $hootdu_customize->get_options('settings');
 ...
 * @return array|false
 */
function hootdu_sortlist( $rawvalue, $returnid = false ) {
