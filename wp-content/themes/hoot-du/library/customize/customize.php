...
require_once ( hootdu_data()->libdir . 'customize/functions.php' );
require_once ( hootdu_data()->libdir . 'customize/interface.php' );
require_once ( hootdu_data()->libdir . 'customize/sanitization.php' );

/** Include custom controls **/
foreach ( glob( hootdu_data()->libdir . 'customize/control-*.php' ) as $file_path ) {
	...
	public function add_options( $options = array() ) {
		foreach ( array( 'settings', 'sections', 'panels' ) as $key ) {
			if ( isset( $options[ $key ] ) && is_array( $options[ $key ] ) && !empty( $options[ $key ] ) ) {
				...
				$add_options = apply_filters( "hootdu_customize_add_{$key}" , $add_options );
...
do_action( 'hootdu_customize_loaded' );