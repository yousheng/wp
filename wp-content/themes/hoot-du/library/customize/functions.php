 ...
 * @return array
 */
function hootdu_customize_prepare_settings( $settings ){
	...
	foreach ( $settings as $key => $setting ) {
		if ( isset( $setting['type'] ) ) :
			...
			$new_value = array();
			$new_value = apply_filters( 'hootdu_customize_prepare_settings', $new_value, $key, $setting, $count );
			if ( !empty( $new_value ) )
...
add_filter( 'hootdu_customize_add_settings', 'hootdu_customize_prepare_settings', 999 );
 ...
 */
function hootdu_customize_get_default( $setting ) {
 ...
 * @return mixed $default
 */
function hootdu_customize_get_choices( $setting ) {
 ...
 * @return mixed
 */
function hootdu_get_mod( $name, $default = NULL ) {
	...
	if ( is_customize_preview() ) {
		...
		$default = ( $default !== NULL ) ? $default : hootdu_customize_get_default( $name );
		$mod = get_theme_mod( $name, $default );

		return apply_filters( 'hootdu_get_mod_customize', $mod, $name, $default );
		...
		if ( !$mods ) {
			$mods = get_theme_mods();
			$mods = apply_filters( 'hootdu_get_mods', $mods );
		}
		...
		// Return default theme option value specified in customizer settings
			...
			$default = hootdu_customize_get_default( $name );
			if ( !empty( $default ) ) {
				$returnvalue = apply_filters( "theme_mod_{$name}", $default );
			}
		}
		...
		if ( $returnvalue !== false ) {
			...
			// Sanitize Value
			$returnvalue = hootdu_sanitize_get_mod( $returnvalue, $name );
