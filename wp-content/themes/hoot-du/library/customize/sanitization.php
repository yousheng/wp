 ...
 * - for retreiving theme_mod values using 'hootdu_get_mod()'
...
/**
 * Sanitization function for return value of 'hootdu_get_mod()'
 * 
 * @since 3.0.0
 * @access public
 * @param mixed $value
 ...
 * @return mixed
 */
function hootdu_sanitize_get_mod( $value, $name ) {
	...
	if ( isset( $settings[ $name ] ) && isset( $settings[ $name ]['id'] ) ) {
		...
		// Fallback to default functions
			...
			$callback = hootdu_customize_sanitize_callback( $setting['type'], $setting, $name );
 ...
 * Also used to get sanitization function name for sanitizing values for 'hootdu_sanitize_get_mod'
 *
 ...
 * @return string
 */
function hootdu_customize_sanitize_callback( $type, $setting, $name ) {

	$callback = false;

	if ( 'text' == $type )
		$callback = 'hootdu_sanitize_text';

	elseif ( 'textarea' == $type )
		$callback = 'hootdu_sanitize_textarea';

	elseif ( 'url' == $type )
		$callback = 'esc_url';

	elseif ( 'upload' == $type || 'image' == $type )
		$callback = 'hootdu_sanitize_upload';

	elseif ( 'checkbox' == $type )
		$callback = 'hootdu_sanitize_checkbox';

	elseif ( 'range' == $type )
		$callback = 'hootdu_sanitize_range';
	...
	elseif ( 'color' == $type )
		...
		// So we can use 'sanitize_hex_color' instead of 'hootdu_sanitize_color'
		$callback = 'sanitize_hex_color';

	// Customizer specific wrapper
	elseif ( 'select' == $type || 'radio' == $type )
		$callback = 'hootdu_sanitize_customize_enum';
	...
	return apply_filters( 'hootdu_customize_sanitize_callback', $callback, $type, $setting, $name );

}

/**
 * Wrapper function for 'hootdu_sanitize_enum' for customizer settings.
 ...
 */
function hootdu_sanitize_customize_enum( $value, $setting ) {
	$name = '';
	...
	return hootdu_sanitize_enum( $value, $choices );
}