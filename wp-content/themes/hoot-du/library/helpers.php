 ...
 * @param string $arg
 */
if ( !function_exists( 'hootdu_current_theme_supports' ) ):
function hootdu_current_theme_supports( $feature, $arg = '' ) {
	if ( !empty( $arg ) ) {
 ...
 * @return string
 */
if ( !function_exists( 'hootdu_trim' ) ):
function hootdu_trim( $content ) {
 ...
 * @return string
 */
if ( !function_exists( 'hootdu_trim_content' ) ):
function hootdu_trim_content( $raw, $words ) {
	$text = $raw;
 ...
 * @return array
 */
if ( !function_exists( 'hootdu_array_insert' ) ):
function hootdu_array_insert( $insert, $target, $location, $order = 'before' ) {
 ...
 * @return string
 */
if ( !function_exists( 'hootdu_locate_script' ) ):
function hootdu_locate_script( $location, $return = 'uri' ) {
	$location = preg_replace( array(
		'/\.min\.css$/',
		'/\.css$/',
		), '', $location );
	return hootdu_locate_uri( $location, 'js', $return );
}
endif;
 ...
 * @return string
 */
if ( !function_exists( 'hootdu_locate_style' ) ):
function hootdu_locate_style( $location, $return = 'uri' ) {
	$location = preg_replace( array(
		'/\.min\.js$/',
		'/\.js$/',
		), '', $location );
	return hootdu_locate_uri( $location, 'css', $return );
}
endif;
 ...
 * @return string
 */
if ( !function_exists( 'hootdu_locate_uri' ) ):
function hootdu_locate_uri( $location, $type, $return = 'uri' ) {

	$location = str_replace( array(
		hootdu_data()->template_uri . 'premium/',
		hootdu_data()->template_dir . 'premium/',
		hootdu_data()->template_uri,
		hootdu_data()->template_dir,
		), '', $location );

	$pattern = apply_filters( 'hootdu_locate_uri_extension_pattern', array( '/\.min\.' . $type . '$/', '/\.' . $type . '$/' ) );
	...
	if ( is_child_theme() ) {
		...
		if ( $loadminified )
			$locations['child-default-min'] = array(
				'path' => hootdu_data()->child_dir . $location . '.min.' . $type,
				'uri'  => hootdu_data()->child_uri . $location . '.min.' . $type,
				);

		$locations['child-default'] = array(
			'path' => hootdu_data()->child_dir . $location . '.' . $type,
			'uri'  => hootdu_data()->child_uri . $location . '.' . $type,
			);
	...
	if ( $loadminified )
		$locations['default-min'] = array(
			'path' => hootdu_data()->template_dir . $location . '.min.' . $type,
			'uri'  => hootdu_data()->template_uri . $location . '.min.' . $type,
			);

	$locations['default'] = array(
		'path' => hootdu_data()->template_dir . $location . '.' . $type,
		'uri'  => hootdu_data()->template_uri . $location . '.' . $type,
		);

	$locations = apply_filters( 'hootdu_locate_uri', $locations, $location, $type, $loadminified );
...
class Hootdu_List {
	...
	static function listlength(){
		return apply_filters( 'hootdu_admin_list_item_count', 75 );
	}
