...
add_action( 'after_setup_theme', 'hootdu_load_textdomains', 5 );

# Filter the textdomain mofile to allow child themes to load the parent theme translation.
add_filter( 'load_textdomain_mofile', 'hootdu_load_textdomain_mofile', 10, 2 );
 ...
 * @return void
 */
function hootdu_load_textdomains() {

	if ( is_child_theme() ) {

		// Load theme textdomain.
		$parent_domain = hootdu_data( 'theme' )->parent()->get( 'TextDomain' );
		$parent_domainpath = hootdu_data( 'theme' )->parent()->get( 'DomainPath' );
		...
		load_theme_textdomain( $textdomain, hootdu_data()->template_dir . $path );

		// Load child theme textdomain.
		$child_domain = hootdu_data( 'theme' )->get( 'TextDomain' );
		$child_domainpath = hootdu_data( 'theme' )->get( 'DomainPath' );
		$textdomain = ( !empty( $child_domain ) ) ? $child_domain : get_template();
		$textdomain = sanitize_key( $textdomain );
		$path = ( !empty( $child_domainpath ) ) ? trim( $child_domainpath, '/' ) : 'languages';
		load_child_theme_textdomain( $textdomain, hootdu_data()->child_dir . $path );

	} else {

		// Load theme textdomain.
		$theme_domain = hootdu_data( 'theme' )->get( 'TextDomain' );
		$theme_domainpath = hootdu_data( 'theme' )->get( 'DomainPath' );
 ...
 * @return string
 */
function hootdu_load_textdomain_mofile( $mofile, $domain ) {
	...
	if ( $domain == $themedomain || $domain == $templatedomain ) {
		...
		$mofile_short = str_replace( array( hootdu_data()->template_dir, hootdu_data()->child_dir ), '', $mofile_short );
 ...
 * @return string
 */
function hootdu_get_language( $locale = '' ) {
 ...
 * not all locales will have a region, so this might actually return the same thing as 'hootdu_get_language()'.
 *
 * @since 3.0.0
 * @access public
 * @param string  $locale
 * @return string
 */
function hootdu_get_region( $locale = '' ) {
