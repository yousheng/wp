 ...
 */
function hootdu_register_sidebar( $args ) {
	...
	// Parse the arguments.
	$args = wp_parse_args( $args, apply_filters( 'hootdu_sidebar_defaults', $defaults ) );

	// Remove action.
	remove_action( 'widgets_init', '__return_false', 95 );

	// Register the sidebar.
	return register_sidebar( apply_filters( 'hootdu_sidebar_args', $args ) );
}
 ...
 * @return string
 */
function hootdu_get_sidebar_name( $sidebar_id ) {
 ...
 * @return bool
 */
function hootdu_widget_exists( $widget ) {
