 ...
 * 
 * $hootdu_style_builder->add will sanitizes the property and adds it using 'wp_add_inline_style()'
 *
 * @since 3.0.0
 * @access public
 ...
 * @return void
 */
function hootdu_add_css_rule( $args ) {

	global $hootdu_style_builder;
	...
	if ( !is_array( $property ) ) {
		...
		$hootdu_style_builder->add( $selector, $property, $value, $idtag, $media, $important, $typography_reset );
		...
		foreach ( $property as $addproperty => $prop_value ) {
			...
			$hootdu_style_builder->add( $selector, $addproperty, $addvalue, $addidtag, $media, $addimportant, $addreset );
		}
 ...
 * @since 3.0.0
 */
class Hootdu_Style_Builder {

	/**
	 * The one instance of Hootdu_Customize.
	 *
	 * @since 3.0.0
	 * @access private
	 * @var Hootdu_Customize The one instance for the singleton.
	 */
	...
	function build_css(){
		...
		do_action( 'hootdu_dynamic_cssrules' );
		...
		$this->dynamiccss = wp_strip_all_tags( apply_filters( 'hootdu_dynamic_css', $css ) );

		/* Print CSS */

		if ( !empty( $this->dynamiccss ) ) {
			$handle = apply_filters( 'hootdu_style_builder_inline_style_handle', 'hootdu-style' );
		...
		switch( $property ):
			...
			case 'border-left-color':
				if ( 'none' == $value || 'transparent' == $value )
					...
					$value = hootdu_sanitize_color( $value );
				break;
			...
			case 'background-repeat':
				$value = hootdu_sanitize_background_repeat( $value );
				break;

			case 'background-position':
				$value = hootdu_sanitize_background_position( $value );
				break;

			case 'background-attachment':
				$value = hootdu_sanitize_background_attachment( $value );
				break;
			...
			case 'font':
				if ( !empty( $value ) ) {
					...
					$value = hootdu_sanitize_fontface( $value );
				} elseif ( !empty( $idtag ) ) {
	...
	function sanitize_background( $idtag, $important = false ) {
		...
		foreach ( $properties as $property ) {
			$prop_value = hootdu_get_mod( "{$idtag}-{$property}" );
			if ( !empty( $prop_value ) )
	...
	function sanitize_backgroundarray( $backgroundarray, $idtag = '', $important = false ) {
		...
		if ( 'custom' == $type ) {
			...
			// 'hootdu_sanitize_background_pattern' will return 0 if pattern = 0 i.e. user selected the first options of 'No Pattern'
			$pattern = hootdu_sanitize_background_pattern( $pattern );
			if ( $pattern ) {
				$background_image = hootdu_data()->template_uri . $pattern;
		...
		$output = apply_filters( 'hootdu_style_builder_background', $output, $idtag, $important );
	...
	function sanitize_typographyarray( $typographyarray, $idtag = '', $important = false, $reset = false ) {
		...
		$style = ( !empty( $style ) ) ? hootdu_sanitize_font_style( $style ) : '';
		if ( !empty( $style ) ) {
			switch ( $style ) {
				case 'italic':
		...
		$output = apply_filters( 'hootdu_style_builder_typography', $output, $idtag, $important, $reset );
	...
	/**
	 * Instantiate or return the one Hootdu_Customize instance.
	 *
	 * @since 3.0.0
	 * @access public
	 * @return Hootdu_Customize
	 */
...
/* Initialize class */
global $hootdu_style_builder;
$hootdu_style_builder = Hootdu_Style_Builder::get_instance();