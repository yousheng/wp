...
add_action( 'wp_enqueue_scripts', 'hootdu_register_styles', 0 );
...
add_action( 'wp_enqueue_scripts', 'hootdu_enqueue_styles', 12 );

/* Load the development stylsheet (unminified) in script debug mode. */
add_filter( 'stylesheet_uri', 'hootdu_min_stylesheet_uri', 5, 2 );

/* Filters the WP locale stylesheet. */
// Child theme users can add this filter if they need it.
// add_filter( 'locale_stylesheet_uri', 'hootdu_locale_stylesheet_uri', 5 );

/* Load admin stylesheet files */
add_action( 'admin_enqueue_scripts', 'hootdu_register_adminstyles' );
 ...
 * @return void
 */
function hootdu_register_styles() {

	/* Get styles. */
	$styles = hootdu_get_styles();
 ...
 * @return array
 */
function hootdu_get_styles() {

	/* Initialize */
	$styles = array();

	/* If a child theme is active, add the parent theme's style. */
	// Cannot use 'hootdu_locate_style()' as the function will always return child
	...
	if ( is_child_theme() ) {
		...
		if ( $loadminified && file_exists( hootdu_data()->template_dir . "style.min.css" ) )
			$src = hootdu_data()->template_uri . "style.min.css";
		else
			// We can skip file_exists for src as parent style.css will always be there.
			$src = hootdu_data()->template_uri . "style.css";

		$styles['hootdu-style'] = array(
			'src' => $src,
	...
	$handle = ( is_child_theme() ) ? 'hootdu-child-style' : 'hootdu-style';
	$styles[ $handle ] = array(
	...
	return apply_filters( 'hootdu_styles', $styles );
}
 ...
 */
function hootdu_min_stylesheet_uri( $stylesheet_uri, $stylesheet_dir_uri ) {
 ...
 * @return string
 */
function hootdu_locale_stylesheet_uri( $stylesheet_uri ) {
	$locale_style = hootdu_get_locale_style();
 ...
 * @return string
 */
function hootdu_get_locale_style() {

	$styles = array();

	// Get the locale, language, and region.
	$locale = strtolower( str_replace( '_', '-', get_locale() ) );
	$lang   = strtolower( hootdu_get_language() );
	$region = strtolower( hootdu_get_region() );
 ...
 * @return void
 */
function hootdu_register_adminstyles() {
	if ( apply_filters( 'hootdu_force_theme_fa', true, 'admin' ) )
